...
output_txt_path = f".\\SearcTubeResultsAPP\\subjects\\{subject_name}\\{subject_name}_FrameTime.txt"
...
def read_csv(csv_data, HMS_frame_time):
    ...
    HMS_start_time = start_time.strftime("%H:%M:%S.%f")
    HMS_start_time = end_time.strftime("%H:%M:%S.%f")

    # Timestamp列の各アイテムを取り出す
    for index, timestamp_item in csv_data['Timestamp'].iteritems():
        timestamp = datetime.strptime(timestamp_item, "%H:%M:%S.%f")
        HMS_timestamp = timestamp.strftime("%H:%M:%S.%f")
...
def get_frame_times(video_path, output_txt_path):
    ...
    with open(output_txt_path, 'w') as file:
        ...
        for frame_number in range(int(cap.get(cv2.CAP_PROP_FRAME_COUNT))):
            ...
            # HMS_frame_time = frame_time.strftime("%H:%M:%S.%f")
            # read_csv(csv_data, HMS_frame_time)
